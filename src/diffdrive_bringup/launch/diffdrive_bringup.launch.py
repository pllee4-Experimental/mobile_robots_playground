...
def generate_launch_description():
    ...
    declared_arguments = [
        DeclareLaunchArgument(
            ...
            default_value=os.path.join(
                diffdrive_bringup_path, "worlds", "default_world.sdf"
            ),
    ...
    ign_bridge = Node(
        ...
        ros_arguments=[
            "-p",
            f"config_file:={os.path.join(diffdrive_bringup_path, 'config', 'ign_bridge.yaml')}",
        ],
        output="screen",
    )
    ...
    rviz2 = Node(
        ...
        arguments=[
            "-d",
            os.path.join(diffdrive_bringup_path, "rviz", "localization.rviz"),
        ],
        output="screen",
    ...
    xacro_file = os.path.join(
        get_package_share_directory("diffdrive_description"), "urdf", "robot.urdf.xacro"
    )
